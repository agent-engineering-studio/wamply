...
export function FailureInsight({ aiEnabled, failedCount }: Props) {
  ...
  async function run() {
    ...
    setError(null);
    try {
      const res = await apiFetch("/messages/failure-insight", { method: "POST" });
  ...
  return (
    <div className="mb-4 rounded-card border border-indigo-500/30 bg-indigo-500/5 p-4">
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          ...
          <div className="mt-0.5 text-[11.5px] text-slate-400">
            {insight
              ? "Claude ha esaminato gli ultimi 30 fallimenti."
              : `Hai ${failedCount} messaggi falliti. Chiedi all'AI di identificare il pattern.`}
          </div>
        </div>
        {!insight && (
          <button
      ...
      {insight && (
        <div className="mt-3 space-y-2.5">
          ...
          {insight.root_causes.length > 0 && (
            <div>
              <div className="mb-1 text-[10.5px] font-semibold uppercase tracking-wider text-rose-300">
                Cause probabili
              </div>
