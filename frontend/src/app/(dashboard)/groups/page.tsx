    ...
    <>
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-[11px] text-slate-400">{groups.length} gruppi</p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        {groups.map((g) => (
          <div key={g.id} className="rounded-card border border-slate-800 bg-brand-navy-light p-4 shadow-card transition-all hover:border-brand-teal hover:shadow-md cursor-pointer">
            ...
            <div className="mt-3 flex items-center justify-between border-t border-slate-800 pt-2.5">
              ...
              <span className="text-[11px] font-medium text-brand-teal">Gestisci</span>
            </div>
          </div>
        ))}
      </div>
